...
import {
  Gamepad2, RefreshCw, ChevronRight, X, Eye, Check, Lightbulb, Trophy,
  ArrowUp, ArrowDown, ArrowLeft, ArrowRight,
} from "lucide-react";
import { PUZZLES } from "@/lib/puzzles";

type Game = "zip" | "2048" | "lights" | "riddle";

/* ============================================================ ZIP ============================================================ */
function genZip() {
  const n = 5;
  const count = 5 + Math.floor(Math.random() * 2); // 5-6 checkpoints
  // snake order, random corner / orientation for variety
  const order: number[] = [];
  ...
  for (let k = 0; k < count; k++) {
    const idx = Math.round((k * (total - 1)) / (count - 1));
    numbers[order[idx]] = k + 1;
  }
...
function Zip() {
  ...
  const reset = () => setPath([]);
  const fresh = () => { setPuz(genZip()); setPath([]); };

  const click = (i: number) => {
    if (path.length === 0) { if (numbers[i] === 1) setPath([i]); return; }
    if (path[path.length - 1] === i) { setPath(path.slice(0, -1)); return; } // undo tip
    if (path.includes(i)) return;
    if (!adjacent(path[path.length - 1], i)) return;
    // can't step onto a numbered cell out of order
    const num = numbers[i];
    if (num) {
      const seqSoFar = path.filter((c) => numbers[c]).map((c) => numbers[c]);
      const lastNum = seqSoFar.length ? seqSoFar[seqSoFar.length - 1] : 0;
      if (num !== lastNum + 1) return;
    }
    setPath([...path, i]);
  };

  const seq = path.filter((c) => numbers[c]).map((c) => numbers[c]);
  const won = path.length === n * n && seq.length === count && seq.every((v, idx) => v === idx + 1);

  return (
    <div className="flex flex-col items-center">
      <p className="text-xs text-slate-500 mb-3 text-center">Connect <b>1 → {count}</b> in order, filling every square. Tap to draw, tap the tip to undo.</p>
      <div className="relative" style={{ width: size, height: size }}>
        <svg className="absolute inset-0 pointer-events-none" width={size} height={size}>
          {path.length > 1 && (
            <polyline
              points={path.map((i) => { const p = center(i); return `${p.x},${p.y}`; }).join(" ")}
              fill="none" stroke="#6366f1" strokeWidth={10} strokeLinejoin="round" strokeLinecap="round" opacity={0.85}
            />
          )}
        </svg>
        <div className="grid" style={{ gridTemplateColumns: `repeat(${n}, ${CELL}px)`, gap: GAP }}>
          {Array.from({ length: n * n }, (_, i) => {
            ...
            const tip = path[path.length - 1] === i;
            return (
              <button key={i} onClick={() => click(i)}
                className={`relative rounded-xl border text-sm font-bold transition ${inPath ? "border-indigo-300 bg-indigo-50" : "border-slate-200 bg-white hover:bg-slate-50"} ${tip ? "ring-2 ring-indigo-400" : ""}`}
      ...
      {won ? (
        <div className="mt-3 flex items-center gap-1.5 text-emerald-600 font-bold text-sm"><Trophy className="w-4 h-4" />Solved!</div>
      ) : (
        <p className="mt-3 text-xs text-slate-400">{path.length}/{n * n} filled</p>
      )}
...
const spawn = (b: number[][]) => {
  ...
  const nb = b.map((x) => x.slice()); nb[r][c] = Math.random() < 0.9 ? 2 : 4; return nb;
};
const init2048 = () => spawn(spawn(Array.from({ length: 4 }, () => [0, 0, 0, 0])));
const moveBoard = (board: number[][], dir: string) => {
  const b = board.map((r) => r.slice());
  if (dir === "left" || dir === "right") {
    for (let r = 0; r < 4; r++) { let line = b[r].slice(); if (dir === "right") line.reverse(); line = slide2048(line); if (dir === "right") line.reverse(); b[r] = line; }
  } else {
    for (let c = 0; c < 4; c++) { let line = [b[0][c], b[1][c], b[2][c], b[3][c]]; if (dir === "down") line.reverse(); line = slide2048(line); if (dir === "down") line.reverse(); for (let r = 0; r < 4; r++) b[r][c] = line[r]; }
  }
  return b;
};
...
function G2048() {
  ...
  const move = (dir: string) => {
    setBoard((cur) => {
      const moved = moveBoard(cur, dir);
      if (JSON.stringify(moved) === JSON.stringify(cur)) return cur;
      return spawn(moved);
    });
  };
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="flex items-center justify-between w-full max-w-[268px] mb-3">
        <p className="text-xs text-slate-500">Merge to <b>2048</b></p>
        <span className="text-xs font-bold text-slate-700">Best tile: {best}</span>
      </div>
      <div className="grid grid-cols-4 gap-2 p-2 bg-slate-200 rounded-xl">
        {board.flat().map((v, i) => (
          <div key={i} className={`w-14 h-14 rounded-lg grid place-items-center font-bold ${TILE[v] || "bg-indigo-700 text-white"} ${v >= 1000 ? "text-sm" : "text-lg"}`}>{v || ""}</div>
        ))}
      </div>
      {best >= 2048 && <div className="mt-3 flex items-center gap-1.5 text-emerald-600 font-bold text-sm"><Trophy className="w-4 h-4" />You hit 2048!</div>}
...
const genLights = () => {
  const N = 5;
  let g = Array.from({ length: N }, () => Array(N).fill(false));
  const toggle = (grid: boolean[][], r: number, c: number) => {
    [[0, 0], [1, 0], [-1, 0], [0, 1], [0, -1]].forEach(([dr, dc]) => {
      const nr = r + dr, nc = c + dc;
      if (nr >= 0 && nr < N && nc >= 0 && nc < N) grid[nr][nc] = !grid[nr][nc];
    });
  };
  for (let k = 0; k < 6; k++) toggle(g, Math.floor(Math.random() * N), Math.floor(Math.random() * N));
  return g;
};
function Lights() {
  const N = 5;
  ...
  return (
    <div className="flex flex-col items-center">
      <p className="text-xs text-slate-500 mb-3 text-center">Turn <b>all the lights off</b>. Tapping a tile flips it and its neighbours.</p>
      <div className="grid gap-1.5" style={{ gridTemplateColumns: `repeat(${N}, 46px)` }}>
        {grid.map((row, r) => row.map((on, c) => (
          <button key={`${r}-${c}`} onClick={() => press(r, c)}
            className={`w-[46px] h-[46px] rounded-lg transition ${on ? "bg-gradient-to-br from-amber-300 to-orange-400 shadow-inner" : "bg-slate-200 hover:bg-slate-300"}`} />
        )))}
      </div>
      {won ? <div className="mt-3 flex items-center gap-1.5 text-emerald-600 font-bold text-sm"><Trophy className="w-4 h-4" />Cleared in {moves} moves!</div> : <p className="mt-3 text-xs text-slate-400">{moves} moves</p>}
...
function Riddle() {
  ...
  const check = () => {
    ...
    setVerdict(ok ? "right" : "wrong"); if (ok) setReveal(true);
  };
  return (
    <div>
      <div className="flex items-center justify-between mb-2"><span className="text-[11px] font-bold uppercase tracking-wide text-indigo-400">{p.cat} · #{p.id}</span></div>
      <p className="text-[15px] font-medium text-slate-800 leading-relaxed min-h-[4rem]">{p.q}</p>
      <div className="mt-3 flex gap-2">
    ...
    return (
      <div className="shrink-0 self-start sticky top-6">
        <button onClick={() => setOpen(true)} className="flex flex-col items-center gap-2 px-2.5 py-4 rounded-2xl bg-gradient-to-b from-indigo-600 to-violet-600 text-white shadow-lg hover:shadow-xl transition">
          ...
          <span className="text-xs font-bold tracking-wide" style={{ writingMode: "vertical-rl" }}>Games</span>
        </button>
      </div>
    );
  }
        ...
        <div className="h-full bg-slate-50 rounded-[1.6rem] flex flex-col overflow-hidden">
          ...
          <div className="px-4 pt-6 pb-3 bg-gradient-to-r from-indigo-600 to-violet-600 text-white">
            ...
            <div className="flex gap-1.5">
              {GAMES.map((g) => (
                <button key={g.key} onClick={() => setGame(g.key)} className={`flex-1 px-2 py-1.5 rounded-lg text-xs font-semibold transition ${game === g.key ? "bg-white text-indigo-700" : "bg-white/10 text-white/80 hover:bg-white/20"}`}>{g.label}</button>
              ))}
            </div>
          </div>
          {/* body */}
