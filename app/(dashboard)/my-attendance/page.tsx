...
import { isoDow, DEFAULT_WORKING_DAYS, fetchWorkingDays } from "@/lib/schedule";
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
        ...
        <div className="px-5 py-4 border-t border-gray-100 flex gap-2">
          ...
          <button onClick={handleSave} disabled={saving}
            className="flex-1 py-2 bg-[#0f172a] text-white text-sm font-semibold rounded-xl hover:bg-[#1e293b] transition-colors flex items-center justify-center gap-2 disabled:opacity-60">
...
// ─── CSV Upload Modal ─────────────────────────────────────────────────────────
function CsvUploadModal({ orgId, onDone, onClose }: { orgId: string; onDone: () => void; onClose: () => void }) {
  const supabase = useSupabase();
    ...
    reader.onload = async e => {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        const { data: emp } = await supabase.from("employees")
          .select("id").eq("employee_code", empCode).eq("org_id", orgId).maybeSingle();
  ...
  const downloadTemplate = () => {
    const csv = "employee_code,date,status,check_in,check_out,notes\nSHA001,2026-05-01,present,09:00,18:00,\nSHA002,2026-05-01,absent,,,Sick";
...
function WeeklyView({
  ...
  const offDay = (d: Date) => !workingDays.includes(((d.getDay() + 6) % 7) + 1);
...
function EmployeeDetailView({
  ...
  const isOff = (d: Date) => !workingDays.includes(((d.getDay() + 6) % 7) + 1);
  ...
  const workDays = days.filter(d => workingDays.includes(((d.getDay() + 6) % 7) + 1) && isoDate(d) <= todayStr).length;
...
function DepartmentSummaryView({
  ...
  const workDaysPassed = days.filter(d => workingDays.includes(((d.getDay() + 6) % 7) + 1) && isoDate(d) <= todayStr).length;
...
export default function AttendancePage() {
  ...
  const isOffDay = (d: Date) => !workingDays.includes(isoDow(d));
  ...
  const resolveOrg = useCallback(async (): Promise<string> => {
    ...
    if (typeof window !== "undefined") {
      ...
      if (stored) { setOrgId(stored); return stored; }
    }
    ...
    const { data: firstOrg } = await supabase.from("organizations").select("id").order("created_at").limit(1).maybeSingle();
    if (firstOrg?.id) { setOrgId(firstOrg.id); return firstOrg.id; }
    return "";
  }, [orgId, supabase]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Org default work schedule (working weekdays) — drives % denominators + weekly-off shading
      const wd = await fetchWorkingDays(supabase, oid, "");
      setWorkingDays(wd);
      ...
      const [{ data: emps }, { data: att }] = await Promise.all([
        supabase.from("employees").select("id, full_name, department, employee_code")
          .eq("org_id", oid).eq("status", "active").order("full_name"),
        supabase.from("attendance").select("*")
          .eq("org_id", oid).gte("date", from).lte("date", to),
      ]);

      setEmployees((emps ?? []) as Employee[]);
      const map: AttendanceMap = {};
      (att ?? []).forEach((r: AttendanceRecord) => { map[`${r.employee_id}_${r.date}`] = r; });
  ...
  }, [resolveOrg, supabase, year, month]);

  useEffect(() => { fetchData(); }, [fetchData]);

  // ── Listen for org switch ────────────────────────────────────────────────────
  useEffect(() => {
    const handler = (e: Event) => {
      const { orgId: id } = (e as CustomEvent).detail;
      setOrgId(id);
      setAttendanceMap({});
      setEmployees([]);
    };
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`attendance:${orgId}:${year}:${month}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "attendance", filter: `org_id=eq.${orgId}` },
        payload => {
          const r = (payload.new || payload.old) as AttendanceRecord;
          if (r?.date) {
            setAttendanceMap(prev => {
              const next = { ...prev };
              if (payload.eventType === "DELETE") delete next[`${r.employee_id}_${r.date}`];
              else next[`${r.employee_id}_${r.date}`] = r;
              return next;
            });
          }
        })
  ...
  const markAllPresent = async () => {
    ...
    const { error } = await supabase.from("attendance").insert(
      unmarked.map(e => ({
        org_id: oid, employee_id: e.id, date: todayStr,
  ...
  return (
    <div className="flex flex-col gap-5">
            ...
            <table className="w-full" style={{ minWidth: `${200 + days.length * 40}px` }}>
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredEmployees.map(emp => {
                  ...
                  const workDaysPassed = days.filter(d => workingDays.includes(isoDow(d)) && new Date(isoDate(d)) <= today).length;
      ...
      {csvModal && (
        <CsvUploadModal
          orgId={orgId}
          onDone={() => { fetchData(); setCsvModal(false); }}
